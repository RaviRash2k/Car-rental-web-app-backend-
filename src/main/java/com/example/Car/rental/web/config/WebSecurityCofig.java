...
public class WebSecurityCofig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable).authorizeHttpRequests(request ->
                request.requestMatchers("/api/auth/**").permitAll() // Publicly accessible endpoints for authentication
                        .requestMatchers("/api/auth/admin/**").hasAnyAuthority(UserRole.ADMIN.name())
                        .requestMatchers("/api/auth/customer/**").hasAnyAuthority(UserRole.CUSTOMER.name())
